...
actor ThumbnailCacheManager {
    ...
    internal func getCacheDirectory() throws -> URL {
    ...
    func storeThumbnail(_ image: CGImage,
        ...
        // Update metadata with the original video URL
        try await updateMetadata(for: videoHash, thumbnailURL: thumbnailURL, timestamp: timestamp, quality: quality, videoURL: videoURL)
    ...
    private func updateMetadata(for videoHash: String,
        ...
        if fileManager.fileExists(atPath: metadataURL.path) {
            ...
            var existingMetadata = try decoder.decode(ThumbnailCacheMetadata.self, from: data)
            // Update only the lastAccessDate to preserve the original video URL
            metadata = ThumbnailCacheMetadata(
                version: existingMetadata.version,
                videoHash: existingMetadata.videoHash,
                modificationDate: existingMetadata.modificationDate,
                parameters: existingMetadata.parameters,
                thumbnails: existingMetadata.thumbnails,
                lastAccessDate: Date(),
            ...
            metadata = ThumbnailCacheMetadata(
                ...
                parameters: ThumbnailParameters.standard,
                thumbnails: [],
                lastAccessDate: Date(),
                originalURL: videoURL
            )
