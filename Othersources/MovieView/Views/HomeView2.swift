...
struct HomeView: View {
    ...
    enum SidebarItem: Hashable {
        ...
        var icon: String {
            ...
            case .cachedMovies: return "film.stack.fill"
            }
        }
        ...
        var title: String {
            ...
            case .cachedMovies: return "Cached Movies"
            }
        }
    }
    ...
    var body: some View {
        NavigationSplitView {
            ...
            List(selection: $selectedSidebarItem) {
                ...
                Section("History") {
                    ForEach([SidebarItem.recentVideos, .cacheStatus, .cachedMovies], id: \.self) { item in
                        Label(item.title, systemImage: item.icon)
                            .tag(item)
                    }
                }
    ...
    // Add computed property for filtered movies
    private var filteredMovies: [MovieFile] {
        if searchText.isEmpty {
            return folderProcessor.movies
        } else {
            return folderProcessor.movies.filter {
