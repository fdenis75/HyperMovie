...
public final class GenerationCoordinator {
    ...
    public func generateMosaics(
        ...
        defer {
          ...
          let batchId = try DatabaseManager.shared.createMosaicBatch(
            settingsJson: """
            {
                "width": \(width),
                "density": "\(density)",
                ...
                "options": {
                    ...
                    "useSeparateFolder": \(options.useSeparateFolder),
                    "addFullPath": \(options.addFullPath),
                    "addBorder": \(options.addBorder),
                    "addShadow": \(options.addShadow),
                    "borderWidth": \(options.borderWidth),
                    "orientation": "\(options.orientation)",
                    "useAutoLayout": \(options.useAutoLayout)
                }
            }
            """
        )
                ...
                for (video, outputDirectory) in files {
                    ...
                    group.addTask {
                        do {
                            ...
                            let config = ProcessingConfiguration(width: width, density: density, format: format, previewDuration: options.minimumDuration,separateFolders: options.useSeparateFolder,addFullPath: options.addFullPath,addBorder: options.addBorder,addShadow: options.addShadow,borderWidth: options.borderWidth,orientation: options.orientation, useAutoLayout: options.useAutoLayout)
                            let result = try await mosaicGenerator.processSingleFile(video: video, output: outputDirectory, config: config, batchId: batchId)
    ...
    */
    ...
    private func updateProgress(
        ...
        if processedFiles > 0 {
            let averageTimePerFile = elapsedTime / Double(processedFiles)
            let remainingFiles = Double(totalFiles - processedFiles)
